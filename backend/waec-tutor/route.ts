...
// KaTeX rendering will respect inline ($...$) and display ($$...$$) delimiters
marked.use(
  markedKatex({
    throwOnError: false, // Don't throw errors for invalid LaTeX
    // The extension respects KaTeX delimiters; do not set unknown options here.
  })
);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsingPrompt = `You are an intelligent assistant that extracts specific information from a user's request.
      From the user's query: "${userQuery}", extract the subject, year, and question number for a WAEC exam.
    ...
    if (subjectMatch && yearMatch && questionNumberMatch) {
      ...
      if (!response.ok) {
        ...
        if (!Array.isArray(allQuestions)) {
          ...
          prompt = `You are a WAEC tutor. I received an unexpected response from the question database for ${subjectMatch} ${yearMatch}. Please ask the user to try again shortly.`;
        } else {
    ...
    if (retrievedQuestion) {
      ...
      prompt = `You are a world-class WAEC tutor. A student has asked for a specific past question which has been retrieved from a live database.
      ...
      prompt = `You are a world-class WAEC tutor AI. The user sent a message: "${userQuery}".
    ...
    console.error("Error in WAEC Tutor API:", error);
    const errorMessage =
